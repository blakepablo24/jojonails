...
$query = "SELECT * FROM eyelash_extensions";
$db_query = mysqli_query($connection, $query);
confirm($db_query);
while ($row = mysqli_fetch_assoc($db_query)) {
    $treatment_id = $row['eyelash_extensions_id'];
    $treatment_title = $row['eyelash_extensions_title'];
    $treatment_duration = $row['eyelash_extensions_duration'];
    $treatment_price = $row['eyelash_extensions_price'];
