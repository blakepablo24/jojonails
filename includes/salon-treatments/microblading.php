...
$query = "SELECT * FROM microblading";
$db_query = mysqli_query($connection, $query);
confirm($db_query);
while ($row = mysqli_fetch_assoc($db_query)) {
    $treatment_id = $row['microblading_id'];
    $treatment_title = $row['microblading_title'];
    $treatment_duration = $row['microblading_duration'];
    $treatment_price = $row['microblading_price'];
